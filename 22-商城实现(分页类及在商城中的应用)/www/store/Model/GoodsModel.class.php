...
class GoodsModel extends Model{
    ...
    */
    public function catGoods($category_id,$offset=0,$limit=5){
        ...
        $sql = 'select goods_id,goods_name,shop_price,market_price,thumb_img from ' . $this->table . ' where category_id in (' . $in . ') order by add_time limit ' . $offset . ',' . $limit;
    ...
    获取分类商品总数
    */
    public function catGoodsCount($category_id){
        $category = new CategoryModel();
        $categorys = $category->select();// 取出所有栏目
        $sons = $category->getCategoryTree($categorys,$category_id);// 取出给定栏目的子孙栏目
        ...
        $sql = 'select count(*) from goods where category_id in (' . $in . ')';
